...
// 모바일일때 1, 모바일이 아닐때가 0 (가로크기 500기준)
let mobsts = 0;
...
$(() => {
    ...
    scAct.each((idx, ele) => {
        scpos[idx] =
            $(ele).offset().top - 206 - hw;
        // 시작위치보정: 원래위치=상단높이-윈도우절반
        // $(ele) 제이쿼리 선택필수
        // offset().top -> 맨 위에서부터 top위치값
    }); // each 메서드
    ...
    $(window).scroll(() => {
        ...
        // 1. 슬림메뉴 클래스 on적용!
        // 기준위치는 스크롤위치 100px이상
        // mobsts === 0 모바일이 아닐 때만 들어옴!
        if (scTop >= 100 && mobsts === 0) { // 100px 이상
            topA.addClass("on");
